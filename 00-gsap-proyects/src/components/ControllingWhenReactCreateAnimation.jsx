...
export const ControllingWhenReactCreateAnimation = () => {
    const app = useRef();
    ...
    useLayoutEffect(() => {
      const ctx = gsap.context(() => {
        gsap.to(".box-1", { rotation: "+=360",backgroundColor:"purple" });
      }, app);
      return () => ctx.revert();
    }, []);
    ...
    useLayoutEffect(() => {
      const ctx = gsap.context(() => {
        gsap.to(".box-2", { rotation: "+=360",backgroundColor:"blue" });
      }, app);
      return () => ctx.revert();
    }, [delayedCount]);
    ...
    useLayoutEffect(() => {
      const ctx = gsap.context(() => {
        gsap.to(".box-3", { rotation: "+=360", backgroundColor:"red"});
      }, app);
      return () => ctx.revert();
    });
